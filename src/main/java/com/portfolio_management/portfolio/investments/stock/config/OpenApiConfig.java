package com.portfolio_management.portfolio.investments.stock.config;

import io.swagger.v3.oas.models.OpenAPI;
import io.swagger.v3.oas.models.info.Contact;
import io.swagger.v3.oas.models.info.Info;
import io.swagger.v3.oas.models.info.License;
import io.swagger.v3.oas.models.servers.Server;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;

import java.util.List;

@Configuration
public class OpenApiConfig {

    @Bean
    public OpenAPI portfolioOpenAPI() {
        return new OpenAPI()
                .info(new Info()
                        .title("Portfolio Manager API")
                        .description("REST API for managing a financial portfolio — stocks, bonds, crypto, cash. " +
                                "Includes live market data via Finnhub, price alerts, audit history, " +
                                "and AI-powered morning predictions.")
                        .version("1.0.0")
                        .contact(new Contact().name("Portfolio Manager Team"))
                        .license(new License().name("MIT")))
                .servers(List.of(new Server().url("http://localhost:8080").description("Local Dev")));
    }
}

